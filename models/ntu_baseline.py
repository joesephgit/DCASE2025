...
class SelfAttention(nn.Module):
    def __init__(self, embed_dim, num_heads, spatial_dim):
        super(SelfAttention, self).__init__()
        self.multihead_attn = nn.MultiheadAttention(embed_dim, num_heads, batch_first = True)
        self.fc = nn.Linear(spatial_dim, embed_dim)
        self.fc_out = nn.Linear(embed_dim, spatial_dim)
        self.batch = nn.BatchNorm1d(spatial_dim)
        self.dropout = nn.Dropout(0.3)
        
    def forward(self, x):
        # x: (batch_size, channel, embed_dim)
        x = x.permute(0, 2, 1)  # Transform to (batch dize, elements, channel)
        x = self.fc(self.dropout(x)) # prepare for attention (batch size, elements, embed dim)
        #x = self.batch(x.transpose(1, 2)).transpose(1, 2)
        attn_output, _ = self.multihead_attn(x, x, x) # (batch size, elements, embed dim)
        x = self.fc_out((attn_output))
        x = (x.transpose(1, 2))
        #attn_output = attn_output.transpose(1, 2) # (batch, embed dim, elements)
        #attn_output = torch.mean(x, dim=2)
        attn_output = x[:,:, -1]
...
def initialize_weights(m):
    if isinstance(m, nn.Conv2d):
        #nn.init.kaiming_normal_(m.weight, mode="fan_out") # frrom fan_out
        #nn.init.kaiming_normal_(m.weight, mode="fan_in") # frrom fan_out
...
class ChannelRand(nn.Module):
    def __init__(self):
        super(ChannelRand, self).__init__()
        #self.groups = groups

    def forward(self, x):
        perm = torch.randperm(x.size(1))  # Generate random permutation
        x = x[:, perm, ...]        
        return x       
