...
#from pytorch_lightning.strategies import DeepSpeedStrategy
    ...
    # get gpu status and assign process to gpu with more memory
gpus = GPUtil.getGPUs()

if gpus[0].memoryUsed > gpus[1].memoryUsed:
    active_gpu = 1
...
for gpu in gpus:
    print(f"GPU {gpu.id}: {gpu.memoryUsed:.1f} MB used / {gpu.memoryTotal:.1f} MB total")
print(f"Using gpu {active_gpu}")   
...
from dataset.dcase25 import get_training_set, get_test_set, open_h5, close_h5
...
def train(config):
    ...
    hf_in = open_h5('C:/bl1/h5py_audio_wav')

    roll_samples = config.orig_sample_rate * config.roll_sec
    train_ds = get_training_set(config.subset, dir_prob = False, hf_in = hf_in, device=None, roll=roll_samples)
    train_dl = DataLoader(
        dataset=train_ds,
    ...
    test_ds = get_test_set(hf_in = hf_in, device=None)
    test_dl = DataLoader(
        dataset=test_ds,
        worker_init_fn=worker_init_fn,
        num_workers=config.num_workers,
    ...
    '''checkpoint_callback = pl.callbacks.ModelCheckpoint(
        monitor="val_loss",          # Metric to monitor (must match your validation loop)
        mode="min",                  # We want the *lowest* val_loss
        save_top_k=1,                # Save only the best checkpoint
#        filename="best-model-{epoch:02d}-{val_loss:.4f}",  # Optional custom filename
    )'''
    ...
    copy2(os.path.basename(__file__), ckp_path.parent)
    copy2('d:/asc_25/models/ntu_baseline.py', ckp_path.parent)
    copy2('d:/asc_25/dataset/dcase25_ntu.py', ckp_path.parent)        
